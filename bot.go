...
func handleEmojiCommandForDM(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if err != nil {
		...
		sendMessageByBot(ctx, b, update, "Не удалось получить информацию о боте")
		return
	}
	...
	if err := prepareWorkingEnvironment(ctx, b, update, emojiArgs); err != nil {
		...
		sendMessageByBot(ctx, b, update, message)
		return
	}
	...
	if emojiPack == nil {
		...
		if err != nil {
			...
			sendMessageByBot(ctx, b, update, "Не удалось создать запись в базе данных")
			return
		}
	}
	...
	for {
		...
		if err != nil {
			...
			sendMessageByBot(ctx, b, update, fmt.Sprintf("Ошибка при обработке видео: %s", err.Error()))
			return
		}
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "STICKERSET_INVALID") {
				sendMessageByBot(ctx, b, update, fmt.Sprintf("Не получилось создать некоторые эмодзи. Попробуйте еще раз, либо измените файл."))
				return
			}
			...
			if strings.Contains(err.Error(), "retry_after") {
				...
				if waitTime > 0 {
					...
					sendMessageByBot(ctx, b, update, fmt.Sprintf("Вы сможете создать пак только через %.0f минуты", dur.Minutes()))
					return
				}
			}

			sendMessageByBot(ctx, b, update, fmt.Sprintf("%s", err.Error()))
			return
		}
	...
	sendMessageByBot(ctx, b, update, fmt.Sprintf("Ваш пак\n%s", "https://t.me/addemoji/"+emojiArgs.PackLink))
